...
import time
class OfflineValidationC3TPT():
    def __init__(self,TestID='',ProjectJson='',TracePath='',mode='TPT'):
        print("TPT val")
        self.TestID =TestID
        ...
        try:
            self.update_TClogs("General",f"Validation started for : {self.JMOIData[self.mode][TestID]['Testcase_Name']}")
    ...
    #To split the flows(PD-SD) and consider last flow for the Validation
        ...
        try:
            ...
            if len(limit)>1:
                ...
                while id < limit[1]:
                    ...
                    if 'Ping Initiated' in self.file_list[id].get('pktType'):
                        # print('pd',id)
                        #find Shutdown
                ...
                self.update_TClogs("General",f"Considerd flow {'-'.join(map(str,packets))} for the validation.")
                return packets
        except Exception as e:
                ...
                if pktstatus == True:
                    ...
                    while rid <= limit[1]:
                        if self.file_list[rid].get('isFWTestermessage') == False and self.file_list[rid].get('isTesterPkt') == True:
                            ...
                            break
                        rid+=1
                #     if len(self.timing_map[packets[seq[seqpos]]['PhaseID']][seq[seqpos]]) == 0: self.timing_map[packets[seq[seqpos]]['PhaseID']][seq[seqpos]].append([[id,self.file_list[id].get('startTime'),self.file_list[id].get('stopTime')]])
                #     # Check for PT pkts
    ...
    def timing_checks(self):
        AllTimings={"twake":[],"tstart":[],"tsilent":[],"tresponse":[]}
        #Twake check--------------------------------------------------------------------------
        remarks = []
        tol= self.Json_Def['twake'] if 'twake' not in self.Json_TC else self.Json_TC['twake']
        AllTimings['twake_exp'] = str(tol[0])+'-'+str(tol[1])
        AllTimings['twake']='NA'
        ...
        try:
            remarks = []
            TresList = []
            tol= self.Json_Def['tresponse'] if 'tresponse' not in self.Json_TC else self.Json_TC['tresponse']
            for phase in self.timing_map:
            ...
            if len(remarks)>0:
                ...
                if len(TresList)>0:
                    ...
                    AllTimings['tresponse_res'] ='NA'
                    AllTimings['tresponse']='No Values'
            AllTimings['tresponse_exp'] = str(round(tol[0]-tol[1],2))+'-'+str(round(tol[0]+tol[1],2))
        ...
        try:
            ...
            if len(self.Json_TC['App_Measures'])>0:
                for apmes in self.Json_TC['App_Measures']:
                    ...
                    # print(self.Json_TC['App_Measures'][apmes])
                    if apmes in ["V1"]:
                        #Get prevouis flow
                        PD = self.GetPacketDetails(packet='Ping Initiated',limit=[self.timing_map['General']['PD'][0]-1,0])
                        if len(PD)>0:
                            res = self.CalculateVoltTwindow(PD[2],self.AllChannelData,winsize=[0,0])
                            AllMeasures[apmes] =  res[0]
                    elif apmes in ["V2"]:
                        res = self.CalculateVoltTwindow(self.timing_map['General']['PD'][0],self.AllChannelData,winsize=[0,0])
                        AllMeasures[apmes] =  res[0]
                    elif apmes in ["MaxPower"]:
                        id = 0
                        powerlist = []
                        while id < len(self.file_list)-1:
                            if '8 bit Received Power' in self.file_list[id].get("pktType"):
                                res= GeneralMethods.GetFloatFromStr(self.file_list[id].get("value"))
                                powerlist.append(res[0])
                            id+=1
                        if len(powerlist)>0:AllMeasures[apmes]=max(powerlist)
                    # apply Validations
        ...
        try:
            if len(self.Json_TC['other_checks_details'])>0:
                for apchk in self.Json_TC['other_checks_details']:
                    if apchk not in AllOtherChecks:
                        AllOtherChecks[apchk]=None
                        if 'expected' in self.Json_TC['other_checks_details'][apchk]:
                            if type(self.Json_TC['other_checks_details'][apchk]['expected']) == list:
                                if len(self.Json_TC['other_checks_details'][apchk]['expected'])==2:
                                    AllOtherChecks[apchk+'_exp']=','.join(map(str,self.Json_TC['other_checks_details'][apchk]['expected']))
                                else:AllOtherChecks[apchk+'_exp']=self.Json_TC['other_checks_details'][apchk]['expected'][0]
                            else:  AllOtherChecks[apchk+'_exp']=self.Json_TC['other_checks_details'][apchk]['expected']
                        else: AllOtherChecks[apchk+'_exp']=None
                        AllOtherChecks[apchk+'_res']='Fail'
                    if apchk in ['SSvalue1']:
                        #Get prevouis flow
                        SS = self.GetPacketDetails(packet='Signal strength',limit=[(self.timing_map['General']['PD'][0]-1),0])
                        if len(SS)>1:
                            floatlist = GeneralMethods.GetFloatFromStr(self.file_list[SS[2]].get('value'))
                            AllOtherChecks[apchk]=floatlist[0]
                            AllOtherChecks[apchk+'_res']='Pass' if AllOtherChecks[apchk] < AllOtherChecks[apchk+'_exp'] else 'Fail'
                    elif apchk in ['SSvalue2']:
                        if len(SS)>1:
                            floatlist = GeneralMethods.GetFloatFromStr(self.file_list[self.timing_map['Ping']['SS'][0][0][0]].get('value'))
                            AllOtherChecks[apchk]=floatlist[0]
                            AllOtherChecks[apchk+'_res']='Pass' if AllOtherChecks[apchk] < AllOtherChecks[apchk+'_exp'] else 'Fail'
                    elif apchk in ['SScomparison']:
                        AllOtherChecks[apchk]=f"{AllOtherChecks['SSvalue1']}<{AllOtherChecks['SSvalue2']}<255"
                        AllOtherChecks[apchk+'_res'] = 'Pass' if AllOtherChecks['SSvalue1'] < AllOtherChecks['SSvalue2'] and AllOtherChecks['SSvalue2'] < 255 else 'Fail'
                    elif apchk in ['PacketCheck']:
                        self.PacketCheck(AllOtherChecks)
                    elif apchk in ['BitsCheck']:
                        self.BitsCheck(AllOtherChecks)
                    elif apchk in ['PowerRemove']:
                        id = 0
                        PowerRemoveTime=[]
                        nextping =[]
                        while id < len(self.file_list)-1:
                            if 'Signal strength' in self.file_list[id]['pktType']:
                                #Get SD
                                SD = self.GetPacketDetails(packet='Shutdown',limit=[id,len(self.file_list)-1])
                                if len(SD)>1:
                                    PowerRemoveTime.append(round(((self.file_list[SD[2]]['startTime'] - self.file_list[id-1]['startTime'])*1000),2))
                                    #Get Next ping
                                    PD = self.GetPacketDetails(packet='Ping Initiated',limit=[SD[2],len(self.file_list)-1])
                                    if len(PD)>1:
                                        nextping.append(round(((self.file_list[PD[2]]['startTime'] - self.file_list[SD[2]]['stopTime'])*1000),2))
                                        id=PD[2]+1
                                    else:
                                        id=SD[2]+1
                                        break
                                else:
                                    PowerRemoveTime.append(round((self.file_list[len(self.file_list)-1]['startTime'] - self.file_list[id-1]['startTime'])*1000),2)
                                    break
                            else:id+=1
                        if len(PowerRemoveTime)>0:
                            res = []
                            for val in PowerRemoveTime:
                                if val <=  AllOtherChecks[apchk+'_exp'][0] and val >= AllOtherChecks[apchk+'_exp'][1]:
                                    res.append('Fail')
                            AllOtherChecks[apchk]=','.join(map(str,PowerRemoveTime))
                            AllOtherChecks[apchk+'_exp'] = f"{AllOtherChecks[apchk+'_exp'][0]}-{AllOtherChecks[apchk+'_exp'][1]}"
                            AllOtherChecks[apchk+'_res']='Fail' if len(res)>0 else 'Pass' 
                        if 'NextPing' in self.Json_TC['other_checks_details']:
                            AllOtherChecks['NextPing']=None
                            AllOtherChecks['NextPing_exp']=self.Json_TC['other_checks_details']['NextPing']['expected'] if 'expected' in self.Json_TC['other_checks_details']['NextPing'] else None
                            AllOtherChecks['NextPing_res']='Fail'
                            res=[]
                            if len(nextping)>0:
                                for val in nextping:
                                    if val <=  AllOtherChecks['NextPing_exp'][0] and val >= AllOtherChecks['NextPing_exp'][1]: res.append('Fail') 
                                AllOtherChecks['NextPing']=','.join(map(str,nextping))
                                AllOtherChecks['NextPing_exp'] = f"{AllOtherChecks['NextPing_exp'][0]}-{AllOtherChecks['NextPing_exp'][1]}"
                                AllOtherChecks['NextPing_res']='Fail' if len(res)>0 else 'Pass'
                    elif apchk in ['PacketLimit']:
                        #Ensure only the expected packets are recevied after the limit
                        limit2 = None
                        receviedPkts = []
                        id = self.flows[0]
                        LimitPkt = self.Json_TC['other_checks_details'][apchk]['LimitPkt']
                        while id < self.flows[1]:
                            if LimitPkt[0][1] is None:
                                if LimitPkt[0][0] in self.file_list[id]['pktType']:
                                    if len(LimitPkt)==1:
                                        limit2=[id+1,self.flows[1]]
                                        break
                                    else:
                                        tid = id+1
                                        while tid < self.flows[1]:
                                            if LimitPkt[1][1] is None:
                                                if LimitPkt[1][0] in self.file_list[tid]['pktType']:
                                                    limit2=[id+1,tid-1]
                                                    break
                                            else:
                                                if LimitPkt[1][0] in self.file_list[tid]['pktType'] and LimitPkt[1][1] in self.file_list[tid]['value']:
                                                    limit2=[id+1,tid-1]
                                                    break
                                            tid+=1
                            id+=1
                        if limit2 is not None:
                            id = limit2[0]
                            while id < limit2[1]:
                                if self.file_list[id]['isTesterPkt'] == False and self.file_list[id]['isTesterPkt']==False:
                                    if self.file_list[id]['pktType'] not in receviedPkts: receviedPkts.append(self.file_list[id]['pktType'])
                                id+=1
                            if len(receviedPkts)>0:
                                AllOtherChecks[apchk] = ','.join(receviedPkts)
                                res=[]
                                for Rpkt in receviedPkts:
                                    if any(i not in Rpkt for i in self.Json_TC['other_checks_details'][apchk]['expected']):
                                        res.append('Fail')
                                if len(res)>0:AllOtherChecks[apchk+'_res']='Pass'
                    elif apchk in ['SRQParameter']:
                        id = self.flows[0]
                        SRQParam = []
                        while id <= self.flows[1]:
                            if 'Specific Request' in self.file_list[id]['pktType']:
                                SRQParam.append(self.file_list[id]['header_Payload']['childelement'][0]['childelement'][0]['sRawData'])
                            id+=1
                        if len(SRQParam)>0:
                            AllOtherChecks[apchk] = ','.join(SRQParam)
                            exp = list(int(res,16) for res in self.Json_TC['other_checks_details'][apchk]['expected'])
                            AllOtherChecks[apchk+'_res'] = 'Pass' if all(int(res,16) in exp for res in SRQParam) else 'Fail'
                    elif apchk in ['SRQReserved']:
                        srqpkt = self.GetPacketDetails(packet="Specific Request",value=self.Json_TC['other_checks_details'][apchk]['value'],limit=self.flows)
                        if len(srqpkt)>1:
                            res =[]
                            val = []
                            #find and check all reserve bits
                            for d1 in self.file_list[srqpkt[2]]['header_Payload']['childelement']:
                                for d2 in d1['childelement']:
                                    if 'Reserved' in d2['sDecodedValue']:
                                        val.append(d2['sRawData'])
                                        if d2['sRawData'] != self.Json_TC['other_checks_details'][apchk]['expected'][0]:
                                            res.append(str(self.file_list[id]['pktType'])+'@index='+str(id)+':'+str(d2['sDecodedValue'])+str(d2['sRawData']))
                            AllOtherChecks[apchk]=f"Found SRQ/{self.Json_TC['other_checks_details'][apchk]['value']} @ {srqpkt[2]}, reserved bits value:{','.join(val)}"
                            AllOtherChecks[apchk+'_exp']=f"SRQ/{self.Json_TC['other_checks_details'][apchk]['value']}, reserved bits value: {self.Json_TC['other_checks_details'][apchk]['expected'][0]}"
                            AllOtherChecks[apchk+'_res'] ="Fail" if len(res)>0 else 'Pass'
                        else:
                            AllOtherChecks[apchk]=f"SRQ/{self.Json_TC['other_checks_details'][apchk]['value']} not found"
                            AllOtherChecks[apchk+'_exp'] = f"SRQ/{self.Json_TC['other_checks_details'][apchk]['value']}"
                            AllOtherChecks[apchk+'_res']="Pass"
                    elif apchk in ['SRQResponse']:
                        srqpkt = self.GetPacketDetails(packet="Specific Request",value=self.Json_TC['other_checks_details'][apchk]['value'],limit=self.flows)
                        if len(srqpkt)>1:
                            #Get the respone
                            AllOtherChecks[apchk+'_exp'] = f"SRQ/{self.Json_TC['other_checks_details'][apchk]['value']}, Response:{self.Json_TC['other_checks_details'][apchk]['expected'][0]}"
                            id = srqpkt[2]+1
                            while id < self.flows[1]:
                                if self.file_list[id]['isTesterPkt'] == True and self.file_list[id]['isFWTestermessage'] == False:
                                    AllOtherChecks[apchk]=f"SRQ/{self.Json_TC['other_checks_details'][apchk]['value']}@{srqpkt[2]}, Response:{self.file_list[id]['pktType']}"
                                    if self.Json_TC['other_checks_details'][apchk]['expected'][0] in self.file_list[id]['pktType']:AllOtherChecks[apchk+'_res']="Pass"
                                    break
                                elif self.file_list[id]['isTesterPkt'] == False and self.file_list[id]['isFWTestermessage'] == False:break
                                id+=1
                        else:
                            AllOtherChecks[apchk]=f"SRQ/{self.Json_TC['other_checks_details'][apchk]['value']} not found"
                            AllOtherChecks[apchk+'_exp'] = f"SRQ/{self.Json_TC['other_checks_details'][apchk]['value']}"
    ...
    def CheckFinalResult(self):
        ...
        if len(checklist)==0:
            ...
            for check in checklist:
                if check in self.timing_map['Timings']:
                    if self.timing_map['Timings'][check+'_res']=='Fail':res.append('Fail')
    ...
    def UpdateToJsonReport(self):
        ...
        if testPresent==False:
            ...
            self.conRep.update_file(self.conRepData)
    def BitsCheck(self,AllOtherChecks):
        reslt = []
        resvalue =[]
        expvalue = []
        for BITSck in self.Json_TC['other_checks_details']['BitsCheck']:
            # print(BITSck)
            if  BITSck['refPrevious'] ==True:
                limit = [0,self.timing_map['General']['PD'][0]-1]
            else:
                if 'refNextAll' in BITSck:
                    limit = [self.timing_map['General']['SD'][0],len(self.file_list)-1]
                else:
                    limit = [self.timing_map['General']['PD'][0],self.timing_map['General']['SD'][0]]
            # if BITSck['refPrevious']  == True:
            ...
            if pktindex != 0:
                for ck in BITSck['Checks']:
                    ...
                    if ck=='SRQrpr_parameter':
                        expvalue.append(str(BITSck['packet'][0])+'_'+str(BITSck['packet'][1])+':SRQrpr_parameter='+str(BITSck['Checks'][ck]['expected']))
                        if self.file_list[pktindex]['header_Payload']['childelement'][1]['childelement'][0]['sRawData'] != BITSck['Checks'][ck]['expected']:
                             reslt.append('Fail')
                        resvalue.append(str(BITSck['packet'][0])+'_'+str(BITSck['packet'][1])+':SRQrpr_parameter='+str(self.file_list[pktindex]['header_Payload']['childelement'][1]['childelement'][0]['sRawData']))
    ...
    def PacketCheck(self,AllOtherChecks):
        print('Pktchk')
        reslt = []
        resvalue =[]
        expvalue = []
        for pkt in self.Json_TC['other_checks_details']['PacketCheck']:
            ...
            if 'refCustom' in pkt:
                limit = self.RefCustomeLimitforPkt(pkt)
            else:
                if  pkt['refPrevious'] ==True:
                    limit = [0,self.timing_map['General']['PD'][0]]
                else:
                            ...
                            if pkt['phase'] != None:
                                if pkt['phase'] == self.file_list[id].get('description'):
                                    ...
                                    while rid < limit[1]:
                                        ...
                                        if self.file_list[rid].get('isTesterPkt') == True and self.file_list[rid].get('isFWTestermessage')== False:
    ...
    #General Functions--------------------------------------------------------------------------------------------
    #To get testcase name by the Test ID from MOIjson
    # def GetTestNameByTestid(self,TestID):
    #     for test in self.JMOIData[self.mode]:
    #         if TestID == 
    #Create log releated to a testcase validation steps.
